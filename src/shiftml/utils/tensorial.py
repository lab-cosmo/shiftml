...
def symmetrize(predictions):
    """
    Given an array of shape (N_samples, 9) where each row represents a flattened 3x3 tensor,
    this function returns the symmetric part of each tensor, also flattened to (N_samples, 9).
    
    Parameters:
    predictions (np.ndarray): Array of shape (N_samples, 9)
    
    Returns:
    np.ndarray: Array of shape (N_samples, 9) containing the symmetric parts of the tensors.
    """
...
T_sym_np = np.array([
    ...
    [0,     0,     -1/2*(-np.sqrt(2)), 0, 0,     0, 1/2*(-np.sqrt(2)), 0,     0    ], #   must be Y_1_-1
